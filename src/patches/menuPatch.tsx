...
const _patchMenu = () => {
  const menuNode = findInReactTree(
    reactTree,
    (node) => node?.memoizedProps?.navID == "MainNavMenuContainer"
  );
    ...
    if (patchedInnerMenu) {
      ...
      afterPatch(
        ...
        (_: any, ret: any) => {
          ...
          const browserPosition = Number.parseInt(
            localStorage.getItem("DECKCORD_MENU_POSITION") || ("3" as string)
          );
