...
class WebRTCServer:
    ...
    def start_pipeline(self, create_offer=True, audio_pt=96, video_pt=97):
        ...
        desc = PIPELINE_DESC.format(
            video_pt=video_pt, audio_pt=audio_pt, monitor=audio_monitor
        )
        self.pipe = Gst.parse_launch(desc)
        self.webrtc = self.pipe.get_by_name("send")
        self.webrtc.connect(
            "on-negotiation-needed", self.on_negotiation_needed, create_offer
        )
            ...
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                if "offer" in data:
                    ...
                    if not self.webrtc:
                        ...
                        pts = get_payload_types(
                            sdpmsg, video_encoding="VP8", audio_encoding="OPUS"
                        )
                        assert "VP8" in pts
                        assert "OPUS" in pts
                        self.start_pipeline(
