...
class User:
    ...
    def from_vc(self, data):
        usr = User(
            {"id": data["userId"], "username": "", "discriminator": None, "avatar": ""}
        )
...
class StoreAccess:
    ...
    async def _store_access_request(self, command, id="", **kwargs):
        ...
        await self.ws.send_json(
            {"type": command, "id": id, "increment": self.request_increment, **kwargs}
        )
    ...
    async def post_screenshot(self, channel_id, data):
        return await self._store_access_request(
            "$screenshot", channel_id=channel_id, attachment_b64=data
        )
